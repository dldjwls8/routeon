...
> 최종 검토: 2026-06-06 (v1.0.94 기준, 백엔드 라우터 리팩터링 회귀 수정 — DB 변경 없음)
...
> v1.0.94는 `backend/schemas.py` 분리 이후 라우터 import 누락과 카카오 모빌리티 캐시 초기화 문제를 수정한 런타임 안정화 버전이다. 신규 테이블·컬럼·ENUM 변경은 없다.
