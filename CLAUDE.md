...
- 기사 수동 상태는 `운행가능/휴무`, 차량 수동 상태는 `가용/정비`만 제공한다. 진행 중 Trip이 연결된 기사·차량은 상세 입력과 삭제 작업을 잠그며 서버도 같은 규칙을 검증한다.
- 기사·차량·고객·오더 검색은 `compositionstart/compositionend`를 처리해 한글 IME 조합 중 재렌더링하지 않는다. 조합 완료 후 재렌더링 시 검색창 포커스와 커서 위치를 복원한다.
...
- 운행관제는 좌측 정사각형 지도, 우측 확대된 요약·차량 위치 패널을 한 viewport에 표시한다. 현재 `in_progress` Trip에 연결된 차량만 목록·지도에 표시하고, 위치 WS 수신 시 좌표 행과 마커를 즉시 갱신한다. 별도 새로고침 버튼은 두지 않는다.
...
- 대시보드는 1440×900 기준 본문 스크롤 없이 핵심 위젯·요약 지도·오더 목록이 보이도록 구성한다. 바로가기는 1~3개를 선택해 `localStorage('dashboardQuickLinks')`에 저장한다.

dashboard.html 오더·배차 UI:
- `오더관리 > 오더접수`는 `정보 입력 → 대기열 확인 → 일괄 저장` 3단계를 표시한다. 첫 번째와 추가 오더 모두 상차·하차 경로 카드와 화주·희망 도착·연락처 영역을 동일하게 가진다. 최종 저장 버튼은 우측 접수 대기열 하단에 배치하며 1440×768 이상에서는 페이지와 입력 영역에 세로 스크롤 없이 전체 작업이 보여야 한다.
- 오더 접수의 화주는 계약 고객, 수신처는 실제 하차 고객을 뜻한다. 희망 도착은 `YYYY-MM-DD`, `HH:MM` 텍스트 입력을 사용하고 혼적 여부는 접수하지 않으며 배차관리에서 결정한다.
- `상차지 추가`, `하차지 추가`로 만든 항목은 기본 상·하차 카드와 같은 제목·순번·장소·화물 입력 구조를 사용한다.
- `오더관리 > 접수창`은 엑셀 업로드 버튼 옆 `양식 다운로드` 버튼을 제공한다. 템플릿은 다중 상·하차 입력을 위해 `상차지1~3/상차화물/상차규격`, `하차지1~3/하차수취인/하차화물/하차규격`, `화주명`, `연락처`, `희망도착일시`, `혼재여부` 헤더를 사용한다. 기본 예시 행은 한 행에 상차·하차 한 쌍만 채워 그대로 업로드할 때 접수 대기열 1건이 생성된다. 기존 단일 `상차지`, `하차지`, `수취인`, `화물종류`, `규격` 헤더도 계속 읽는다.
...
- 오더 상세의 `상·하차` 탭은 `cargo_id` 원본 필드명 대신 `화물 ID`를 사용하고 `RO-...-화물N` 형식으로 표시한다. `지도` 탭은 등록된 모든 상차·하차 좌표를 역할별 핀으로 표시한다.
...
- 오더 상세는 우측 컨테이너에서 조회 전용으로 열리고 `수정`을 눌러야 입력이 활성화되며 버튼은 `저장`으로 바뀐다. 별도 수정 모달은 사용하지 않으며 완료·취소 오더는 조회 전용이다.
- `오더관리 > 배차관리`는 미배정 오더와 가용 기사·연결 차량을 선택한 뒤 `기사에게 배정`으로 기사별 묶음을 만든다. 오더 1건도 같은 흐름으로 처리하며 별도 단건·수동 화면은 없다.
- 배차관리 상단은 `오더 선택 → 기사 선택 → 배정 확인` 단계와 현재 선택·배정 건수를 표시한다. 기사·연결 차량은 오더 목록과 유사한 행 구조로 표시하며, 기사 선택 카드와 배정·배차 실행 카드는 분리한다. 배정 오더 칩을 누르면 실행 전 개별 배정을 취소할 수 있다.
...
- `배차 실행`은 기사별 배정 묶음마다 `/trips/auto-dispatch`를 호출해 사용자가 지정한 기사·차량 연결을 보존한다. 좌표 없는 오더는 미배정 결과로 표시한다.
- 배차관리 화면은 좌측 `미배정 오더`, 우측 상단 `기사·차량 선택`, 우측 하단 `배정 및 실행`, 하단 전체 `차량별 방문 순서·미배정 결과` 구조다. 데스크톱에서는 페이지 스크롤 없이 표시하고 목록이 많으면 각 컨테이너 내부만 스크롤한다.
...
- 한글 검색 입력은 `compositionstart`/`compositionend`를 처리하고 조합 종료 직후 발생하는 중복 `input` 이벤트를 한 번 무시한다. 검색 중 재렌더링하더라도 최종 조합 문자열과 커서 위치를 보존한다.
- 고객관리 메인 탭은 hover 세부 메뉴를 노출하지 않는다. 화면 내부의 고객 목록/상세 탭만 사용한다.
- 캘린더는 월 이동·연월 제목과 범례를 중앙 정렬하고 목록은 `datetime` 기준 최신순으로 표시한다. 날짜와 시간을 함께 출력하며 유형은 별도 컬럼에 두고 ID에는 `TR-YYMMDD-NNN` 운행번호 또는 표시용 오더번호만 사용한다.
