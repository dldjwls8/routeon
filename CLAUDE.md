...
- `window resize` 이벤트(rAF 디바운스)를 전역에서 감지해 지도 페이지(`isMapPage()`)에서는 `kakao.maps.event.trigger(map, 'resize')`와 `applyLiveMapFixedView()`를 다시 호출한다. 화면 비율이 바뀌어 `.control-map-card`/`.dash-map-card` 크기가 달라져도 지도가 컨테이너 크기에 맞춰 고정 표시된다.
