...
- 일정 간트는 날짜 선택·이전 날·다음 날·오늘 이동을 지원하고 선택 날짜의 운행만 06–21시 타임라인에 표시한다.
...
- `오더관리 > 오더접수`는 `정보 입력 → 대기열 확인 → 일괄 저장` 3단계를 표시한다. 상차·하차 입력은 한 경로 그룹으로 묶고 화주·희망 도착·연락처·혼적 여부는 별도 오더 정보 영역에 둔다. 최종 저장 버튼은 우측 접수 대기열 하단에 배치하며 1440×768 이상에서는 페이지와 입력 영역에 세로 스크롤 없이 전체 작업이 보여야 한다.
...
- `오더관리 > 오더목록`은 오더번호·화주·상하차지·화물 통합 검색을 제공한다. 행 클릭을 상세 조회, 체크박스를 다중 선택으로 사용하며 현재 페이지 선택/해제와 선택 해제 버튼을 제공한다. 선택한 `접수` 상태 오더만 `오더관리 > 배차관리`로 전달한다.
...
- 배차관리 상단은 `오더 선택 → 기사 선택 → 배정 확인` 단계와 현재 선택·배정 건수를 표시한다. 기사 카드의 배정 오더 칩을 누르면 실행 전 개별 배정을 취소할 수 있다.
...
- 배차관리 화면은 상단 좌측 `미배정 오더`, 상단 우측 `기사·차량 선택`, 하단 `차량별 방문 순서·미배정 결과` 구조다. 데스크톱에서는 페이지 스크롤 없이 세 영역을 한 화면에 표시하고 목록이 많으면 각 컨테이너 내부만 스크롤한다.
...
- 캘린더는 월 이동·연월 제목과 범례를 중앙 정렬하고 목록은 `datetime` 기준 최신순으로 표시한다. 날짜와 시간을 함께 출력하며 원본 UUID 대신 `운행 XXXXXXXX`, 표시용 오더번호를 사용한다.
- 간트 날짜는 `Date#getFullYear/getMonth/getDate` 기반 로컬 `YYYY-MM-DD`로 계산한다. `toISOString()`을 날짜 이동에 사용하지 않는다.
