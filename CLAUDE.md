...
- 대시보드는 1440×900 기준 한 화면에 핵심 위젯·요약 지도·오더 목록이 들어오도록 구성하되, 창 비율이 달라져 내용이 넘칠 때는 `.dash-layout`이 `overflow-y: auto`로 스크롤된다. `.dash-layout`은 `align-items: start` grid라 `.dash-left`/`.dash-right`가 행 높이로 늘어나지 않고 각자 콘텐츠 높이만큼만 차지하므로(컬럼별 `overflow-y: auto`는 실제로 작동하지 않음), 1100px 미만에서 1열로 쌓일 때처럼 두 컬럼 합산 높이가 영역을 넘으면 `.dash-layout` 자체가 스크롤되어야 footer를 포함한 하단 콘텐츠가 잘리지 않는다. 바로가기는 1~3개를 선택해 `localStorage('dashboardQuickLinks')`에 저장한다.
...
- 오더 상세는 우측 컨테이너에서 조회 전용으로 열리고 `수정`을 눌러야 입력이 활성화되며 버튼은 `저장`으로 바뀐다. 별도 수정 모달은 사용하지 않으며 `완료`·`취소` 상태만 조회 전용이고 그 외(`접수`·`수락대기`·`운행중`)는 모두 인라인 수정이 가능하다(`orderIsEditable`).
- 오더의 화주는 자유 텍스트 입력을 두지 않고 항상 고객 목록에서 선택한다. 오더 상세 수정의 화주 필드와 배차관리 `배송 건 추가` 모달의 화주 필드 모두 `DATA.customers` 기반 select를 사용한다.
- `오더관리 > 배차관리`는 운행(trip) 목록을 조회·수정한다. 배차관리 상세 패널에서 `수정` 버튼을 누르면 기사·차량을 `<select>`로 교체할 수 있으며, `availableDrivers` computed는 `driver_status === '운행가능'`인 기사만 표시하되 현재 선택된 기사는 유지한다. 선택 UI는 `max-width:100%`로 컨테이너를 넘어가지 않도록 스타일을 제한한다.
- 배차관리 상단은 `오더 선택 → 기사 선택 → 배정 확인` 단계와 현재 선택·배정 건수를 표시한다. `미배정 오더`와 `기사·차량 선택` 카드는 모두 동일한 `<table class="bulk-pool-table">` 행 구조·헤더·하단 요약 문구를 사용해 크기와 정렬을 통일한다(`dispatchListTableRows`/`bulkDriverTableRows`). `배정 및 실행`을 별도 카드로 분리하지 않고, 선택한 오더·기사 요약과 `배차 실행` 버튼(`bulk-assign-bar`)은 `기사·차량 선택` 카드가 아니라 `배차 결과`(`#bulkResultsCard`) 컨테이너 상단에 둔다. 클릭 시 선택 항목을 혼적 규칙에 따라 자동 배정한 뒤 곧바로 실행한다. 배정 묶음·결과 요약은 같은 `배차 결과` 패널에서 확인한다. 배정 오더 칩을 누르면 실행 전 개별 배정을 취소할 수 있다.
...
- 배차관리 화면은 좌측 `미배정 오더`, 우측 `기사·차량 선택`, 하단 전체 `배차 결과 — 차량별 방문 순서·미배정`(상단에 선택 오더·기사 건수와 `배차 실행` 버튼 포함) 구조다. 데스크톱에서는 페이지 스크롤 없이 표시하고 목록이 많으면 각 컨테이너 내부만 스크롤한다.
...
- 차량 상세의 삭제 명칭은 `차량 삭제`를 사용한다. `마지막 GPS` 필드는 톤급·차종 등 다른 필드와 동일하게 `<label>+<span>` 한 줄 구조(`vehicleLastGpsDetailHtml`)로 표시해 form-grid 정렬을 통일한다.
- 담당자 상세의 이름·아이디·관리 등급·연락처·가입일은 항상 `disabled` 입력란으로 표시해(`.inline-detail-bd input:disabled`) 고객·기사·차량 상세와 동일하게 비활성 상태를 흐림 스타일로 시각화한다. 수정 가능한 항목은 `화면 접근 권한` 토글뿐이다.
