(function () {
  ...
  let _controlRoutePolyline = null;
  let _controlActivePolyline = null;
  let _controlRouteNodes = [];
  let _controlPolylinePath = [];
  let _controlActiveTripId = null;
  ...
  function gotoPage(main, page) {
    ...
    if (currentPage === 'control-live' && page !== 'control-live') { selectedControlVehicleId = null; clearControlPolyline(); }
    ...
    if (typeof window._onRouteOnGotoPage === 'function') {
      window._onRouteOnGotoPage(main, currentPage);
    }
  }
  ...
  function rowsFromExcelOrder(rawRow) {
    ...
    if (!deliveries.length && legacyDelivery) deliveries.push({ delivery: legacyDelivery, cargo: legacyCargo, tons: legacySize });
  ...
  function applyPageTheme() {
    ...
    if (['trip-stats', 'schedule-calendar', 'schedule-milestones', 'profile'].includes(currentPage)) {
  ...
  function renderDashboard(root) {
    ...
    root.innerHTML = `
      ...
      <div class="page-body-fill">
        <div class="dash-layout" style="height:100%;align-items:stretch">
          ...
          <div class="dash-right" style="display:flex;flex-direction:column;min-height:0">
            <div class="dash-map-card" aria-label="요약 지도"></div>
            <div class="dash-orders-card" style="flex:1;min-height:0;overflow:hidden;display:flex;flex-direction:column">
  ...
  function renderControlLive(root) {
    ...
    root.querySelectorAll('[data-control-vehicle-id]').forEach(row => {
      row.onclick = async () => {
        ...
        // 운행중인 trip 찾아서 폴리라인 로드
        const driver = vehicle?.driverId ? DATA.drivers.find(d => d.id === vehicle.driverId) : null;
        const allTrips = Array.isArray(DATA.trips) ? DATA.trips : (DATA.statsTrips || []);
        const activeTrip = allTrips.find(t =>
          t.status === '운행중' &&
          (Number(t.vehicleId) === selectedControlVehicleId || Number(t.driverId) === Number(driver?.id))
        );
        if (activeTrip) {
          await loadAndShowControlPolyline(activeTrip.id);
        } else {
          clearControlPolyline();
        }
      };
    });
  ...
  function clearControlPolyline() {
    if (_controlRoutePolyline) { _controlRoutePolyline.setMap(null); _controlRoutePolyline = null; }
    if (_controlActivePolyline) { _controlActivePolyline.setMap(null); _controlActivePolyline = null; }
    _controlRouteNodes.forEach(n => n.setMap(null));
    _controlRouteNodes = [];
    _controlPolylinePath = [];
    _controlActiveTripId = null;
  }

  function findClosestPolylineIndex(path, lat, lon) {
    let minDist = Infinity;
    let minIdx = 0;
    const targetLat = Number(lat);
    const targetLon = Number(lon);
    for (let i = 0; i < path.length; i++) {
      const dy = path[i].getLat() - targetLat;
      const dx = path[i].getLng() - targetLon;
  ...
  function updateControlActivePolyline(currentLat, currentLon) {
    if (!_controlPolylinePath.length || !_controlRoutePolyline) return;
    const idx = findClosestPolylineIndex(_controlPolylinePath, currentLat, currentLon);
    const remainingPath = _controlPolylinePath.slice(idx);
    if (_controlActivePolyline) {
      if (remainingPath.length > 1) {
        _controlActivePolyline.setPath(remainingPath);
        _controlActivePolyline.setMap(map);
      } else {
        _controlActivePolyline.setMap(null);
      }
    } else if (remainingPath.length > 1) {
      _controlActivePolyline = new kakao.maps.Polyline({
        path: remainingPath,
        strokeWeight: 4,
        strokeColor: '#4f7cff',
        strokeOpacity: 0.9,
  ...
  async function loadAndShowControlPolyline(tripId) {
    if (!map || !window.kakao?.maps) return;
    clearControlPolyline();
    _controlActiveTripId = tripId;
    ...
    try {
      ...
      _controlPolylinePath = data.polyline.map(p => new kakao.maps.LatLng(p.lat, p.lon));

      // 전체 경로 (낮은 투명도로 배경처럼)
      _controlRoutePolyline = new kakao.maps.Polyline({
        path: _controlPolylinePath,
        strokeWeight: 4,
        strokeColor: '#4f7cff',
        strokeOpacity: 0.25,
        strokeStyle: 'solid',
      ...
      (data.nodes || []).forEach(n => {
        ...
        const overlay = new kakao.maps.CustomOverlay({
          position: new kakao.maps.LatLng(n.lat, n.lon),
          content: `<div style="background:#fff;border:1px solid #ccc;border-radius:4px;padding:2px 6px;font-size:11px;white-space:nowrap;max-width:120px;overflow:hidden;text-overflow:ellipsis">${icon} ${escapeHtml(n.name)}</div>`,
          yAnchor: 1.5,
          map,
        });
        _controlRouteNodes.push(overlay);
      });

      // 현재 위치 기준으로 남은 경로(선명하게) 초기화
      const vehicle = vehicleById(selectedControlVehicleId);
      if (vehicle?.start_lat != null && vehicle?.start_lon != null) {
        updateControlActivePolyline(vehicle.start_lat, vehicle.start_lon);
      }

      const bounds = new kakao.maps.LatLngBounds();
      _controlPolylinePath.forEach(p => bounds.extend(p));
      map.setBounds(bounds);
    } catch (err) {
      console.error('Failed to load control polyline', err);
    }
  }
        ...
        if (msg.type === 'location' || msg.type === 'gps') {
          ...
          if (d) {
            ...
            if (v && driverHasActiveTrip(d.id)) {
              ...
              // 선택된 차량의 폴리라인 실시간 업데이트
              if (selectedControlVehicleId === v.id && _controlActiveTripId) {
                updateControlActivePolyline(msg.lat, msg.lon);
              }
            }
          }
        }
  ...
  window.RouteOnInit = init;
  window.RouteOnGotoPage = gotoPage;
  window.RouteOnLoadRealData = loadRealData;
  window.RouteOnDATA = DATA;
  window.RouteOnRequireAdminSession = requireAdminSession;
})();
