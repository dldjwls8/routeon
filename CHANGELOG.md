...
### 기능 개발 백로그
- [ ] **통계 / 애널리틱스** — 운행 건수, 거리, 시간, 기사별/기간별 집계 대시보드
- [ ] **폴리라인 개선** — 경로선 정확도·스타일 개선 (구간별 색상, 통과 애니메이션 등)
- [ ] **지도 전체 기사 폴리라인 + 위치** — 모든 운행 중 기사의 경로선·현재위치를 지도에 동시 표시
- [ ] **상차지 인근 기사 확인** — 상차지 좌표 기준 반경 내 대기 중 기사 목록 제공
- [ ] **관리자 프리셋 기능** — 자주 쓰는 상차지/하차지 조합을 프리셋으로 저장·재사용
- [ ] **실시간 관제** — 지도 기반 운행 현황 모니터링 (기사 상태, ETA, 이탈 감지 등)
